...
static const at_response_t at_responses_list[] = {
	...
	{ RES_NO_ANSWER, "NO ANSWER", DEF_STR("NO ANSWER\r")},
...
static int at_response_atd(struct pvt* pvt);
static int at_response_ata(struct pvt* pvt);
static int at_response_no_answer (struct pvt * pvt);
/*!
...
*/
			...
			case CMD_AT_CFUN:
				...
				break;
			case CMD_AT_DDSETEX:
				ast_debug (5, "[%s] %s sent successfully\n", PVT_ID(pvt), at_cmd2str (ecmd->cmd));
				if (!pvt->initialized)
				{
					pvt->timeout = DATA_READ_TIMEOUT;
					pvt->initialized = 1;
			...
			case CMD_AT_CHUP:
			case CMD_AT_CHLD_1x:
				//more safe way to huanup a call??
				at_response_no_answer(pvt);
			...
			case CMD_AT_CMGS:
				ast_debug (5, "[%s] Sending sms message in progress\n", PVT_ID(pvt));
				break;
...
static int at_response_orig (struct pvt* pvt, const char* str)
{
	...
	struct cpvt * syscpvt = &pvt->sys_chan;
	...
	ast_debug (2, "[%s] ORIG Received call_index: %d call_type %d\n", PVT_ID(pvt), call_index, call_type);
...
static int at_response_atd(struct pvt* pvt) {
	...
	if (!pvt->initialized)
	{
		pvt->timeout = DATA_READ_TIMEOUT;
		pvt->initialized = 1;
		ast_verb (1, "[%s] Dongle initialized and ready\n", PVT_ID(pvt));
		manager_event_device_status(PVT_ID(pvt), "Initialize");
	}
	//hard coded only one call by currently
    snprintf(bufconn, 10, "^ORIG:%d,%d", 1, 0);
    at_response_orig(pvt, bufconn);
	return 0;
}
...
static int at_response_cend (struct pvt * pvt, const char* str)
{
	...
	{
		ast_debug (5, "[%s] Could not parse all CEND parameters\n", PVT_ID(pvt));
	}

	ast_debug (5,	"[%s] CEND: call_index %d duration %d end_status %d cc_cause %d Line disconnected\n"
...
static int at_response_no_answer (struct pvt * pvt)
{
	...
	call_index = pvt_find_as_current_call(pvt);
	if (call_index <=0){
		ast_log (LOG_ERROR, "[%s] Can't find current call - '%d'\n", PVT_ID(pvt), call_index);
		return 0;
	}
	...
	if (cpvt)
	{
		syscpvt->clcc_check = 0;
		CPVT_RESET_FLAGS(cpvt, CALL_FLAG_NEED_HANGUP);
		PVT_STAT(pvt, calls_duration[cpvt->dir]) += 0;
		change_channel_state(cpvt, CALL_STATE_RELEASED, 2);
		manager_event_cend(PVT_ID(pvt), call_index, 0, 0, 2);
	}
	else
	{
		ast_log (LOG_ERROR, "[%s] NO ANSWER event for unknown call idx '%d'\n", PVT_ID(pvt), call_index);
	}
...
static int at_response_clcc (struct pvt* pvt, char* str)
{
	...
	if (pvt->initialized)
	{
		...
		for(;;)
		{
			...
			{
				ast_debug (5, "[%s] CLCC callidx %u dir %u state %u mode %u mpty %u number %s type %u\n",  PVT_ID(pvt), call_idx, dir, state, mode, mpty, number, type);
				if(mode == CLCC_CALL_TYPE_VOICE && state <= CALL_STATE_WAITING)
				{
					...
					else if(dir == CALL_DIR_INCOMING && (state == CALL_STATE_INCOMING || state == CALL_STATE_WAITING))
					{
						...
						if(pvt_enabled(pvt))
						{
							...
							if(start_pbx(pvt, number, call_idx, state) == 0)
							{
								...
								//there is a voice support, let's send a fake CONN now
					...
					switch(state)
					{
						...
						case CALL_STATE_ACTIVE:{
	                        char bufconn[10];
	                        snprintf(bufconn, 10, "^CONN:%d,%d", call_idx, state);
	                        at_response_conn(pvt, bufconn);
	                        break;
						}
						default:;
					}
...
*/
		...
		AST_LIST_TRAVERSE(&pvt->chans, cpvt, entry)
		{
			if(!CPVT_TEST_FLAG(cpvt, CALL_FLAG_ALIVE)){
				...
				ast_log (LOG_ERROR, "[%s] no answer going here \n", PVT_ID(pvt));
			}
		}
	}
	return 0;
...
*/
			...
			case RES_NO_DIALTONE:
				...
				break;
			case RES_NO_CARRIER:
                ast_debug (1, "[%s] Receive NO CARRIER, SIM800 reconized it to end of call\n", PVT_ID(pvt));
                at_response_no_answer(pvt);
                break;
			case RES_NO_ANSWER:
                ast_debug (1, "[%s] Receive NO ANSWER, SIM800 dial out but called without answer, need to disconnected\n", PVT_ID(pvt));
                at_response_no_answer(pvt);
                break;
			case RES_CPIN:
				/* fatal */
